...
Hosts may instead load the **local** stdio MCP (`parallax-mcp`). That process still does not run models. Keep MCP cwd as the **user project** (use `uv run --project /path/to/Parallax`, never `uv --directory`). Flow: `parallax_interview` → `parallax_brief` → `parallax_prepare` → one fresh context per `perspectives/*.md` → write `reports/<id>.json` → `parallax_synthesize`. See [adapters/README.md](../../adapters/README.md). GitHub pushes do not update a running MCP; the user must upgrade and reload.
